...
const TIME_SLOTS = {
    ...
    5: ['08:00', '10:00', '13:00', '16:00', '20:00'],
...
const normalizeSchedule = (rawSchedule) => {
    ...
    return rawSchedule.map((s, idx) => {
        const time = s.intake_at
            ? formatTimeForInput(s.intake_at)
            : s.time || '08:00';
        return {
            id:       s.id || `db-${idx}`,
            name:     s.name,
...
const SchedulePreview = ({ schedule, onScheduleChange, lang }) => {
    ...
    const handleTimeChange = (id, newTime) => {
        if (!newTime) return;
        onScheduleChange(schedule.map(s =>
            s.id === id ? { ...s, time: newTime, period: getPeriod(newTime) } : s
        ));
    };

    const periods  = ['morning', 'afternoon', 'evening'];
    const byPeriod = periods.reduce((acc, p) => {
        acc[p] = schedule.filter(s => s.period === p);
        return acc;
    }, {});
...
const ExistingFileItem = ({ file }) => {
    ...
    return (
        <a href={file.url} target="_blank" rel="noopener noreferrer" className={`${styles.fileItem} ${styles.fileItemExisting}`}>
            <ft.Icon className={styles.fileIcon} />
            <div className={styles.fileInfo}>
...
const EditPrescriptionPage = () => {
    ...
    useEffect(() => {
        Promise.all([
            ...
            getPrescriptionFiles(id, prescriptionId),
        ])
            .then(([p, wards, prescription, files]) => {
                setPatient(p);
                setAvailableWards(wards);
                setExistingFiles(files);
                setDiagnosis(prescription.diagnosis || '');
                setIcdCode(prescription.icdCode || '');
                setWardId(prescription.wardId || '');
                setComplaints(prescription.complaints || '');
                setAnamnesis(prescription.anamnesis || '');
                setObjectiveStatus(prescription.objectiveStatus || '');
                setRecommendations(prescription.recommendations || '');
                setNotes(prescription.notes || '');
                setMedications(prescription.medications?.length
    ...
    useEffect(() => {
        ...
        medications.forEach((med, idx) => {
            ...
            const alreadyInSchedule = schedule.some(s => s.name === med.medicationName);
            if (alreadyInSchedule) return;

            const slots     = TIME_SLOTS[Math.min(parseInt(med.timesPerDay), 6)] || TIME_SLOTS[1];
            const usedTimes = new Set(schedule.map(s => s.time));

            const newEntries = slots.map((time, slotIdx) => {
                let t = time;
                while (usedTimes.has(t)) t = addMinutes(t, 30);
                usedTimes.add(t);
                return {
                    id:       `auto-${idx}-${slotIdx}-${Date.now()}`,
            ...
            if (newEntries.length > 0) {
                setSchedule(prev =>
                    [...prev, ...newEntries].sort((a, b) => a.time.localeCompare(b.time))
                );
            }
        });
    }, [medications, scheduleInitialized]);
    ...
    const handleMedChange = (i, field, value) => {
        ...
        if (field === 'timesPerDay' && updated[i].medicationName && value) {
            ...
            const slots     = TIME_SLOTS[Math.min(parseInt(value), 6)] || TIME_SLOTS[1];

            setSchedule(prev => {
                const withoutMed = prev.filter(s => s.name !== medName);
                const usedTimes  = new Set(withoutMed.map(s => s.time));

                const newEntries = slots.map((time, idx) => {
                    let t = time;
                    while (usedTimes.has(t)) t = addMinutes(t, 30);
                    usedTimes.add(t);
                    return {
                        id:       `rebuilt-${medName}-${idx}-${Date.now()}`,
                        name:     medName,
                        time:     t,
                        quantity: Number(qty),
                        period:   getPeriod(t),
                ...
                return [...withoutMed, ...newEntries].sort((a, b) => a.time.localeCompare(b.time));
            });
        }
        ...
        if (field === 'quantity' && updated[i].medicationName) {
            const medName = updated[i].medicationName;
            setSchedule(prev => prev.map(s =>
                s.name === medName ? { ...s, quantity: Number(value) } : s
            ));
        }
    };
    ...
    const handleFileAdd = (e) => {
        ...
        setNewFiles(prev => [...prev, ...chosenFiles]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            fd.append('icd_code', icdCode);
            fd.append('wardId', wardId);
            fd.append('complaints', complaints);
            fd.append('anamnesis', anamnesis);
            fd.append('objective_status', objectiveStatus);
            ...
            showToast('success', t('prescription_form.edit_success'));
            ...
            <div className={styles.pageHeader}>
                <div>
                    <div className={styles.breadcrumb}>
                        ...
                        <span className={styles.breadcrumbSep}>/</span>
                        <span>{t('prescription_form.edit_prescription')}</span>
                    </div>
                    <div className={styles.pageTitleRow}>
                        <h2 className={styles.pageTitle}>{t('prescription_form.edit_prescription')}</h2>
                    </div>
                </div>
            ...
            <div className={styles.formBody}>
                    ...
                    <div className={styles.fieldGroup} style={{ marginTop: 16 }}>
                        <div className={styles.labelRow}>
                            <label className={styles.label}>
                                ...
                                <span style={{ fontWeight: 400, color: '#9ca3af', marginLeft: 6, textTransform: 'none', fontSize: 11 }}>
                                    {t('prescription_form.icd_hint')}
                                </span>
                            </label>
                        </div>
                    ...
                    {existingFiles.length > 0 && (
                        <div style={{ marginBottom: 16 }}>
                            <p className={styles.existingFilesLabel}>
                                {t('prescription_form.attached_files')} ({existingFiles.length})
                            </p>
                            <div className={styles.fileList}>
                                {existingFiles.map(f => <ExistingFileItem key={f.file_id} file={f} />)}
                            </div>
                        </div>
                    )}
                ...
                <div className={styles.formSection}>
                    ...
                    <div style={{ marginTop: 24 }}>
                        ...
                        <button className={styles.addMedBtn} onClick={handleAddMed}>
                            <FaPlus size={12} /> {t('prescription_form.add_medication')}
                        </button>
                    </div>
                ...
                <div className={styles.formActions}>
                    <button className={styles.cancelBtn} onClick={() => navigate(`/main/patients/${id}`)}>
                        {t('common.cancel')}
                    </button>
